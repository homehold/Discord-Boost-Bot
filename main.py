...
os.system(f"title Your a retard nigger")    
...
# Capmon captcha solver config.json
...
''')
print(f'''{Style.BRIGHT}{Fore.MAGENTA}                            Tokens Stock: {len(open('input/3m_tokens.txt', 'r').read().splitlines())} Boosts Stock: {len(open('input/3m_tokens.txt', 'r').read().splitlines()) * 2}''' + Fore.RESET)	

def menu():
    invite = input(f"{Style.BRIGHT}{Fore.GREEN}[+]{Fore.MAGENTA} Server: ")
    if ".gg/" in invite:
